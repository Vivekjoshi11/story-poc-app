...
export default function RegisterCustom() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const response = await fetch('/api/registerCustom', {
                method: 'POST',
    ...
    return (
        <div className="max-w-2xl mx-auto p-6">
            <h1 className="text-2xl font-bold mb-4">Register Custom Derivative IP Asset</h1>
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                >
                    {loading ? 'Registering...' : 'Register Derivative & Claim Revenue'}
                </button>
            </form>
            {result && (
                <div className="mt-4 p-4 bg-green-100 rounded text-black">
                    ...
                    <p>Parent Claim Revenue: {JSON.stringify(result.parentClaimRevenue)}</p>
                </div>
            )}
            {error && (
