...
export default function RegisterCommercialDerivative() {
  ...
  const handleClaimRevenue = async () => {
    ...
    if (!result || !result.childIpId) {
      setClaimError('Please register a derivative IP Asset first');
      return;
    }
    ...
    try {
      ...
      // Attempt to claim revenue
      const parentClaimRevenue = await client.royalty.claimAllRevenue({
        ancestorIpId: formData.parentIpId as Address,
        claimer: '0x056c3160301D70f4F8A6e0d8717F8Af3676D1669' as Address, // Parent IP owner address
        childIpIds: [result.childIpId as Address],
        royaltyPolicies: [RoyaltyPolicyLRP], // Update if necessary (e.g., use RoyaltyPolicyLAP)
        currencyTokens: [WIP_TOKEN_ADDRESS],
      });

      // Check if any revenue was claimed (you may need to adjust based on SDK response structure)
      const hasRevenue = parentClaimRevenue && parentClaimRevenue.txHash;

      const serializedClaimResponse = JSON.parse(
        JSON.stringify(
          {
            success: !!hasRevenue,
            receipt: parentClaimRevenue,
            message: hasRevenue ? 'Revenue claimed successfully' : 'No revenue available to claim',
          },
          (key, value) => (typeof value === 'bigint' ? value.toString() : value)
        )
      );

      setClaimResult(serializedClaimResponse);
    } catch (revenueErr: any) {
      setClaimError(`Failed to claim revenue: ${revenueErr.message}`);
      console.error('Revenue claim error:', revenueErr);
    } finally {
  ...
  return (
    <div className="max-w-2xl mx-auto p-6">
      ...
      {claimResult && (
        <div className="mt-4 p-4 bg-green-100 rounded text-black">
          <h2 className="text-lg font-semibold text-black">Revenue Claim Result</h2>
          <p>{claimResult.message}</p>
          {claimResult.success && (
            <p>Revenue Claim Receipt: {JSON.stringify(claimResult.receipt)}</p>
          )}
        </div>
      )}
      ...
      {claimError && (
        <div className="mt-4 p-4 bg-red-100 rounded text-red-800">
          <h2 className="text-lg font-semibold">Revenue Claim Error</h2>
          <p>{claimError}</p>
        </div>
      )}
    </div>
